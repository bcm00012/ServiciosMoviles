...
public class Asynctask_CwdAndList extends AsyncTask<String, ComponentesLista, Integer> {
	...
	public Asynctask_CwdAndList(Context context,Context contentToast, AdaptadorTitulares_pc adaptador_pc,
			ArrayList<ComponentesLista> datos_pc, String destinationFolder) {
		this.adaptador_pc = adaptador_pc;
		this.context = context;
		this.socket = Sokect_Tcp.getInstance().getSocket();
		this.datos_pc = datos_pc;
		this.destinationFolder = destinationFolder;
		this.toast_app = new Toast_app(contentToast);
	}
	...
	protected void onPreExecute() {
		Log.v("Asynctask", "Asynctask_CwdAndList onPreExecute");
		datos_pc.clear();
	...
	protected Integer doInBackground(String... variableNoUsada) {
		Log.v("Asynctask", "Asynctask_CwdAndList doInBackground");
		...
		try {
			...
			out.println("cwd " + destinationFolder);

			if (in.readLine().contentEquals("200 OK") == false) {
			//	toast_app.directorioRaiz();
				Toast.makeText(context,"Usted ya se encuentra en el directorio RAIZ\nC:\\ftpserver", Toast.LENGTH_LONG).show();
			...
			while ((currentLine = in.readLine()) != null) {
				Log.v("Asynctask", "Asynctask_CwdAndList Contenido buffer de entrada: " + currentLine);
	...
	protected void onProgressUpdate(ComponentesLista... currentLine) {
		Log.v("Asynctask", "Asynctask_CwdAndList onProgressUpdate");
	...
	protected void onPostExecute(Integer cantidadProcesados) {
		Log.v("Asynctask", "Asynctask_CwdAndList onPostExecute");
	}
	...
	protected void onCancelled(Integer cantidadProcesados) {
		Log.v("Asynctask", "Asynctask_CwdAndList onCancelled");
